 ...
 * - Sidebar controls
 * - Page routing (maps nav items to render functions)
 * - All 7 page renderers, each calling the backend API
 * - Toast notifications
 */

// ============================================================
//  AUTH GUARD
//  If no token in localStorage, send to login page
// ============================================================
      ...
      : stats.alerts.map(a => `
          <div class="alert-item ${a.status === 'out' ? 'danger' : ''}">
            ...
            <span class="alert-text">
              ...
              ${a.status === 'out'
                ? 'rupture de stock'
                : `seuil bas (${a.current_stock} ${a.unit})`}
            </span>
          </div>`).join('');

    // Recent movements from history (admin only)
      ...
      try {
        ...
        movementsHtml = recent.length === 0
          ...
          : `<table class="data-table">
              <thead><tr><th>Produit</th><th>Type</th><th>Qté</th><th>Par</th></tr></thead>
              <tbody>
                ${recent.map(m => `
                  <tr>
                    <td>${m.product}</td>
                    <td><span class="badge badge-${m.type === 'entry' ? 'entry' : m.type === 'exit' ? 'exit' : 'loss'}">
    ...
    setContent(`
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card">
          ...
          <div class="stat-info">
            <span class="stat-value">${stats.out_of_stock_count}</span>
        ...
        <div class="stat-card">
          ...
          <div class="stat-info">
            <span class="stat-value">${stats.shopping_sessions_this_month}</span>
            <span class="stat-label">Achats ce mois</span>
          </div>
        </div>
      </div>
      ...
      <div class="dashboard-grid">
        <div class="card">
          <h2><i class="fa-solid fa-bell" style="color:var(--color-warning);margin-right:8px;"></i>Alertes stock</h2>
          <br>
          <div class="alert-list">${alertsHtml}</div>
        </div>
        <div class="card">
          <h2><i class="fa-solid fa-clock-rotate-left" style="color:var(--color-info);margin-right:8px;"></i>Derniers mouvements</h2>
          <br>
          ${movementsHtml}
        </div>
      </div>
...
function updateUnit(select) {
  ...
  select.closest('tr').querySelector('.item-unit-label').textContent = unit;
}
...
// ============================================================
//  PAGE: FIN DE JOURNÉE — Version améliorée
//  - Recherche en temps réel
//  - Cartes produits avec stock visuel
//  - Compteur d'articles modifiés
  ...
  try {
      ...
      setContent(`
        <div class="card">
          <div class="empty-state">
            ...
            <p>Aucun produit en stock. Commencez par enregistrer un shopping.</p>
          </div>
        </div>`);
      return;
    }
    ...
    const categorySections = Object.entries(byCategory).map(([cat, prods]) => `
      <div class="eod-category" data-category="${cat}">
        <div class="eod-category-title">
          <i class="fa-solid fa-tag"></i> ${cat}
          <span style="font-size:11px;color:var(--color-text-muted);margin-left:8px;">${prods.length} produit${prods.length>1?'s':''}</span>
        </div>
          ...
          ${prods.map(p => {
            ...
            <div class="eod-card" id="eod-card-${p.id}" data-name="${p.name.toLowerCase()}" data-category="${cat.toLowerCase()}">
              <div class="eod-card-header">
                <span class="eod-product-name">${p.name}</span>
                <span class="eod-stock-badge">${p.current_stock} ${p.unit}</span>
              </div>
              <div class="eod-stock-bar">
                <div style="background:${barColor};height:100%;width:${pct}%;border-radius:99px;transition:width 0.3s;"></div>
              </div>
              <div class="eod-input-row">
                ...
                <div class="eod-qty-wrap">
                  <input type="number"
                    ...
                    onchange="eodOnChange(${p.id})"
                    oninput="eodOnChange(${p.id})">
              ...
              <button class="eod-note-toggle" onclick="eodToggleNote(${p.id})">
                <i class="fa-solid fa-note-sticky"></i> Ajouter une note
              </button>
            </div>`;
          }).join('')}
        </div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;">
          <div>
            <h2 style="display:flex;align-items:center;gap:8px;">
              <i class="fa-solid fa-clipboard-check" style="color:var(--color-accent);"></i>
              Fin de Journée
            </h2>
            <p style="font-size:13px;color:var(--color-text-muted);margin-top:4px;">
              Saisissez les quantités consommées. Les produits non modifiés seront ignorés.
            </p>
          </div>
          <div style="display:flex;align-items:center;gap:10px;">
            <span id="eodCounter" style="font-size:13px;font-weight:600;color:var(--color-text-muted);">0 produit modifié</span>
            <button class="btn btn-primary" onclick="submitEndOfDay()" id="btnSubmitEod" disabled>
              <i class="fa-solid fa-floppy-disk"></i> Valider
            </button>
          </div>
        </div>
        <div style="margin-top:14px;">
          <input type="text" class="form-control" id="eodSearch"
            placeholder="🔍 Rechercher un produit..."
            oninput="eodFilter(this.value)"
      ...
      <div id="eodSummaryModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:200;align-items:center;justify-content:center;padding:16px;">
        <div style="background:#fff;border-radius:16px;padding:28px;width:100%;max-width:480px;max-height:80vh;overflow-y:auto;">
          ...
          <div style="display:flex;gap:12px;margin-top:20px;">
            ...
            <button class="btn btn-primary" style="flex:1;" onclick="confirmEndOfDay()">
...
function eodOnChange(productId) {
  ...
  if (qty > 0) {
    card.classList.add('eod-card-active');
  } else {
    card.classList.remove('eod-card-active');
  }
...
function submitEndOfDay() {
  ...
  if (items.length === 0) { showToast('Aucune sortie saisie.', 'error'); return; }
...
async function confirmEndOfDay() {
  ...
  try {
    ...
    showToast(`${items.length} sortie(s) enregistrée(s) ✅`);
    navigateTo('dashboard');
  } catch(e) {
    showToast('Erreur: ' + e.message, 'error');
  }
...
async function renderLosses() {
  try {
    ...
    const lossRows = losses.length === 0
      ? `<tr><td colspan="6" style="text-align:center;color:var(--color-text-muted);padding:24px;">Aucune perte enregistrée.</td></tr>`
      : losses.map(l => `
          <tr>
    ...
    setContent(`
      <div class="card">
        <h2><i class="fa-solid fa-triangle-exclamation" style="color:var(--color-danger);margin-right:8px;"></i>Signaler une perte</h2>
        <p style="margin-top:4px;">Enregistrez ici tout article cassé, périmé ou manquant.</p>
      </div>
      ...
      <div class="card">
        <div class="form-row">
          <div class="form-group">
            <label>Produit</label>
            <select class="form-control" id="lossProduct" onchange="updateLossUnit(this)">
          ...
          <div class="form-group">
            ...
            <div style="display:flex;align-items:center;gap:8px;">
              <input type="number" class="form-control" id="lossQty" placeholder="0" min="0" step="0.1" style="flex:1;">
              <span id="lossUnit" style="font-size:13px;color:var(--color-text-muted);white-space:nowrap;min-width:40px;"></span>
            </div>
          </div>
        </div>
        <div class="form-group">
          <label>Raison <span style="color:var(--color-danger);">*</span></label>
          <input type="text" class="form-control" id="lossReason" placeholder="ex: Bouteille cassée, produit périmé, vol constaté...">
        </div>
        <div style="display:flex;gap:12px;justify-content:flex-end;">
          <button class="btn btn-primary" onclick="submitLoss()">
            <i class="fa-solid fa-floppy-disk"></i> Enregistrer la perte
          </button>
        </div>
      </div>

      <div class="card">
        <h2><i class="fa-solid fa-list" style="color:var(--color-text-muted);margin-right:8px;"></i>Historique des pertes</h2>
        <br>
        <div class="table-wrap">
          <table class="data-table">
            <thead>
              <tr><th>Date</th><th>Produit</th><th>Quantité</th><th>Raison</th><th>Enregistré par</th></tr>
            </thead>
...
function updateLossUnit(select) {
  const opt = select.options[select.selectedIndex];
  document.getElementById('lossUnit').textContent = opt.dataset.unit || '';
}
    ...
    setContent(`
      <div class="card">
        <div class="empty-state">
          ...
          <p>L'historique complet est réservé à l'administrateur.</p>
        </div>
      </div>
    `);
    return;
    ...
    const lossesHtml = losses.length === 0
      ...
      : losses.map(m => `
          <tr>
            ...
            <td>${m.quantity} ${m.unit}</td>
            <td>${m.manager}</td>
            <td style="color:var(--color-text-muted);font-size:12px;">${m.reason || '—'}</td>
          </tr>`).join('');
...
async function renderProducts() {
  try {
    ...
    const rows = products.length === 0
      ? `<tr><td colspan="6" style="text-align:center;color:var(--color-text-muted);padding:24px;">Aucun produit. Ajoutez-en un ci-dessous.</td></tr>`
      : products.map(p => {
          const status = p.current_stock <= 0
          ...
          return `
            <tr>
              ...
              <td>${p.alert_threshold} ${p.unit}</td>
              <td>${status}</td>
              <td>
                ${user.role === 'admin' ? `
    ...
    const addFormHtml = user.role === 'admin' ? `
      <div class="card">
        <h2><i class="fa-solid fa-plus" style="color:var(--color-accent);margin-right:8px;"></i>Ajouter un produit</h2>
        <br>
        <div class="form-row">
          <div class="form-group">
            <label>Nom du produit</label>
            <input type="text" class="form-control" id="newProductName" placeholder="ex: Coca-Cola 33cl">
          </div>
          <div class="form-group">
        ...
        <div class="form-row">
          <div class="form-group">
            <label>Unité</label>
            <select class="form-control" id="newProductUnit">
          ...
          <div class="form-group">
            ...
            <input type="number" class="form-control" id="newProductThreshold" value="5" min="1">
          </div>
        </div>
    ...
    setContent(`
      <div class="card">
        ...
        <div class="table-wrap" style="margin-top:16px;">
          <table class="data-table">
            <thead>
              <tr><th>Nom</th><th>Catégorie</th><th>Stock actuel</th><th>Seuil alerte</th><th>Statut</th><th></th></tr>
            </thead>
            <tbody>${rows}</tbody>
          </table>
        </div>
        ...
        <div style="background:#fff;border-radius:16px;padding:28px;width:100%;max-width:420px;margin:16px;">
          ...
          <div class="form-group"><label>Catégorie</label>
            <select class="form-control" id="editProductCategory">
              <option>Boissons</option><option>Alcools</option><option>Nourriture</option><option>Condiments</option><option>Autre</option>
            </select>
          </div>
          <div class="form-row">
            <div class="form-group"><label>Unité</label>
              <select class="form-control" id="editProductUnit">
                <option>bouteille</option><option>canette</option><option>litre</option><option>kg</option><option>caisse</option><option>unité</option>
              </select>
            </div>
...
async function submitEditProduct() {
  ...
  const body = {
    ...
    alert_threshold: parseInt(document.getElementById('editProductThreshold').value),
        ...
        <td>
          ${u.id !== currentUserId ? `
            <button class="btn btn-danger btn-sm" onclick="deleteUser(${u.id}, '${u.name}')">
              <i class="fa-solid fa-trash"></i>
            </button>` : '<span style="font-size:12px;color:var(--color-text-muted);">Vous</span>'}
        </td>
      </tr>`).join('');
    ...
    setContent(`
      <div style="display:flex;justify-content:flex-end;margin-bottom:16px;">
        <button class="btn btn-outline" onclick="exportPDF()">
          <i class="fa-solid fa-file-pdf" style="color:var(--color-danger);"></i> Exporter rapport PDF
        </button>
      </div>
      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-icon green"><i class="fa-solid fa-box"></i></div>
          <div class="stat-info"><span class="stat-value">${stats.total_products}</span><span class="stat-label">Produits total</span></div>
        </div>
        <div class="stat-card">
          <div class="stat-icon yellow"><i class="fa-solid fa-triangle-exclamation"></i></div>
          <div class="stat-info"><span class="stat-value">${stats.low_stock_count}</span><span class="stat-label">Stocks faibles</span></div>
        </div>
        <div class="stat-card">
          <div class="stat-icon red"><i class="fa-solid fa-ban"></i></div>
          <div class="stat-info"><span class="stat-value">${stats.out_of_stock_count}</span><span class="stat-label">Ruptures</span></div>
        </div>
        <div class="stat-card">
          <div class="stat-icon blue"><i class="fa-solid fa-cart-shopping"></i></div>
          <div class="stat-info"><span class="stat-value">${stats.shopping_sessions_this_month}</span><span class="stat-label">Achats ce mois</span></div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="table-wrap" style="margin-top:16px;">
          <table class="data-table">
            <thead><tr><th>Nom</th><th>Identifiant</th><th>Rôle</th><th>Créé le</th><th></th></tr></thead>
      ...
      <div class="card">
        <h2><i class="fa-solid fa-user-plus" style="color:var(--color-accent);margin-right:8px;"></i>Créer un utilisateur</h2>
        <br>
        <div class="form-row">
...
async function submitNewUser() {
  ...
  try {
    await api.post('/reports/users', body);
...
//  Full financial dashboard with charts
// ============================================================
async function renderAnalytics() {
  ...
  try {
    ...
    setContent(`
      ...
      <div class="stats-grid" style="grid-template-columns:repeat(5,1fr);">
        <div class="stat-card">
          ...
          <div class="stat-info">
            <span class="stat-value" style="font-size:15px;">${fmt(summary.total_purchase_cost)}</span>
            <span class="stat-label">Coût d'achat total</span>
          </div>
        </div>
        <div class="stat-card">
          <div class="stat-icon blue"><i class="fa-solid fa-chart-line"></i></div>
          <div class="stat-info">
            <span class="stat-value" style="font-size:15px;">${fmt(summary.total_theoretical_gain)}</span>
            <span class="stat-label">Gain théorique</span>
          </div>
        </div>
        <div class="stat-card">
          <div class="stat-icon yellow"><i class="fa-solid fa-triangle-exclamation"></i></div>
          <div class="stat-info">
            <span class="stat-value" style="font-size:15px;">${fmt(summary.total_loss_value)}</span>
            <span class="stat-label">Valeur des pertes</span>
          </div>
        </div>
        <div class="stat-card">
          <div class="stat-icon green"><i class="fa-solid fa-money-bill-wave"></i></div>
          <div class="stat-info">
            <span class="stat-value" style="font-size:15px;">${fmt(summary.total_real_gain)}</span>
            <span class="stat-label">Gain réel</span>
          </div>
        </div>
        <div class="stat-card">
          <div class="stat-icon" style="background:#f0fdf4;color:${marginColor}"><i class="fa-solid fa-scale-balanced"></i></div>
          <div class="stat-info">
            <span class="stat-value" style="font-size:15px;color:${marginColor}">${fmt(summary.total_margin)}</span>
      ...
      <div class="card">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
          ...
          <div style="display:flex;gap:8px;">
            <button class="btn btn-outline btn-sm active-period" id="btnDay" onclick="switchPeriod('day')">Jour</button>
      ...
      <!-- TWO COLUMN: top products + losses -->
      <div class="dashboard-grid">

        <div class="card">
          <h2><i class="fa-solid fa-trophy" style="color:#d97706;margin-right:8px;"></i>Top 5 produits (revenus)</h2>
          <br>
          ${top_products.length === 0
            ? '<p style="color:var(--color-text-muted);font-size:13px;">Aucune donnée de vente.</p>'
            : `<table class="data-table">
                <thead><tr><th>Produit</th><th>Vendu</th><th>Revenu</th></tr></thead>
                <tbody>
                  ${top_products.map(p => `
                    <tr>
                      <td><strong>${p.name}</strong></td>
                      <td>${p.total_sold} ${p.unit}</td>
                      <td style="color:var(--color-accent);font-weight:600;">${fmt(p.revenue)}</td>
                    </tr>`).join('')}
                </tbody>
              </table>`}
        </div>
        ...
        <div class="card">
          <h2><i class="fa-solid fa-triangle-exclamation" style="color:var(--color-danger);margin-right:8px;"></i>Top 5 pertes (valeur)</h2>
          <br>
          ${loss_by_product.length === 0
            ? '<p style="color:var(--color-text-muted);font-size:13px;">Aucune perte enregistrée.</p>'
            : `<table class="data-table">
                <thead><tr><th>Produit</th><th>Perdu</th><th>Valeur</th></tr></thead>
                <tbody>
                  ${loss_by_product.map(p => `
                    <tr>
                      <td><strong>${p.name}</strong></td>
                      <td>${p.total_lost} ${p.unit}</td>
                      <td style="color:var(--color-danger);font-weight:600;">${fmt(p.loss_value)}</td>
                    </tr>`).join('')}
                </tbody>
              </table>`}
        </div>
      ...
      <div class="card">
        <h2><i class="fa-solid fa-chart-bar" style="color:var(--color-accent);margin-right:8px;"></i>Revenus par catégorie</h2>
        <br>
        <canvas id="barChart" height="80"></canvas>
      </div>

      <!-- GAIN vs COST DOUGHNUT -->
      <div class="dashboard-grid">
        <div class="card">
          <h2><i class="fa-solid fa-circle-half-stroke" style="color:var(--color-info);margin-right:8px;"></i>Répartition financière</h2>
          <br>
          <canvas id="doughnutChart" height="200"></canvas>
        </div>
        <div class="card">
          <h2><i class="fa-solid fa-table" style="color:var(--color-text-muted);margin-right:8px;"></i>Résumé par période</h2>
          <br>
          <table class="data-table">
            <thead><tr><th>Période</th><th>Coût</th><th>Gain réel</th><th>Marge</th></tr></thead>
            <tbody>
              ${buildPeriodSummaryRows(daily, weekly, monthly, fmt)}
            </tbody>
          </table>
        </div>
      </div>
    ...
    window._currentPeriod = 'week';

    // Load Chart.js from CDN then draw charts
    loadChartJs(() => {
      drawLineChart(weekly);
      drawBarChart(category_breakdown);
      drawDoughnutChart(summary);
    });
...
function buildPeriodSummaryRows(daily, weekly, monthly, fmt) {
  const sum = arr => arr.reduce((a, b) => ({
    purchase_cost: (a.purchase_cost||0) + (b.purchase_cost||0),
    real_gain: (a.real_gain||0) + (b.real_gain||0),
    margin: (a.margin||0) + (b.margin||0),
  }), {});

  const d = sum(daily.slice(-1));
  const w = sum(weekly);
  const m = sum(monthly);
  ...
  const row = (label, data) => `
    <tr>
      <td>${label}</td>
      <td>${fmt(data.purchase_cost||0)}</td>
      <td style="color:var(--color-accent)">${fmt(data.real_gain||0)}</td>
      <td style="color:${(data.margin||0)>=0?'var(--color-success)':'var(--color-danger)'};font-weight:600;">${fmt(data.margin||0)}</td>
    </tr>`;

  return row("Aujourd'hui", d) + row("7 derniers jours", w) + row("30 derniers jours", m);
}
...
function switchPeriod(period) {
  ...
  ['day','week','month'].forEach(p => {
    ...
    if (btn) {
      btn.className = p === period ? 'btn btn-primary btn-sm' : 'btn btn-outline btn-sm';
    }
  });

  const periodData = period === 'day' ? data.daily : period === 'week' ? data.weekly : data.monthly;
  drawLineChart(periodData);
}
  ...
  canvas._chart = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [
        {
          label: 'Coût d\'achat',
          data: periodData.map(d => d.purchase_cost),
          borderColor: '#ef4444',
          backgroundColor: 'rgba(239,68,68,0.08)',
          tension: 0.4, fill: true, pointRadius: 4,
        },
        {
          label: 'Gain théorique',
          data: periodData.map(d => d.theoretical_gain),
          borderColor: '#3b82f6',
        ...
        {
          label: 'Gain réel',
          data: periodData.map(d => d.real_gain),
          borderColor: '#39812F',
          backgroundColor: 'rgba(57,129,47,0.08)',
          tension: 0.4, fill: true, pointRadius: 4,
        },
        {
          label: 'Marge',
          data: periodData.map(d => d.margin),
          borderColor: '#f59e0b',
...
function drawBarChart(categories) {
  ...
  canvas._chart = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [{
        label: 'Revenu par catégorie (FCFA)',
...
function drawDoughnutChart(summary) {
  ...
  canvas._chart = new Chart(canvas, {
    type: 'doughnut',
    data: {
      labels: ['Coût d\'achat', 'Gain réel', 'Pertes'],
      datasets: [{
        data: [
          summary.total_purchase_cost,
          Math.max(0, summary.total_real_gain),
          summary.total_loss_value,
        ],
        backgroundColor: ['#ef4444', '#39812F', '#f59e0b'],
        borderWidth: 2,
        borderColor: '#fff',
      }]
    },
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: ctx => `${ctx.label}: ${new Intl.NumberFormat('fr-FR').format(ctx.parsed)} FCFA`
          }
        }
      }
    }
...
async function renderIntelligence() {
  ...
  try {
    const [anomalies, profitability, predictions, heatmap] = await Promise.all([
      ...
      api.get('/intelligence/profitability'),
      api.get('/intelligence/predictions'),
    ...
    const anomalyRows = anomalies.length === 0
      ...
      : anomalies.slice(0,10).map(a => `
          <tr>
            ...
            <td style="color:var(--color-danger);font-weight:700;">${a.quantity} ${a.unit}</td>
            <td style="color:var(--color-text-muted);">moy: ${a.average_quantity} ${a.unit}</td>
            <td><span class="badge ${a.severity==='high'?'badge-out':'badge-low'}">${a.ratio}× la moyenne</span></td>
            <td>${a.manager_name}</td>
          </tr>`).join('');

    // ── PROFITABILITY ──
    const gradeColor = g => g==='A'?'var(--color-success)':g==='B'?'var(--color-info)':g==='C'?'var(--color-warning)':'var(--color-danger)';
    const profitRows = profitability.slice(0,10).map(p => `
      <tr>
        <td>
          <div style="display:flex;align-items:center;gap:10px;">
            <span style="width:28px;height:28px;border-radius:50%;background:${gradeColor(p.grade)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;flex-shrink:0;">${p.grade}</span>
            <strong>${p.name}</strong>
          </div>
        </td>
        <td>${p.category}</td>
        <td>${fmt(p.revenue)}</td>
        <td style="color:${p.margin_pct>=0?'var(--color-success)':'var(--color-danger)'};font-weight:600;">${p.margin_pct}%</td>
        <td>
          <div style="background:#f3f4f6;border-radius:99px;height:8px;width:100%;overflow:hidden;">
            <div style="background:${gradeColor(p.grade)};height:100%;width:${p.score}%;border-radius:99px;"></div>
          </div>
          <span style="font-size:11px;color:var(--color-text-muted);">${p.score}/100</span>
        </td>
        <td>${fmt(p.loss_value)}</td>
      </tr>`).join('');

    // ── PREDICTIONS ──
    const predRows = predictions.filter(p => p.days_until_empty !== null).slice(0,8).map(p => {
      const color = p.urgency==='critical'?'var(--color-danger)':p.urgency==='warning'?'var(--color-warning)':'var(--color-success)';
      const icon  = p.urgency==='critical'?'fa-circle-xmark':p.urgency==='warning'?'fa-triangle-exclamation':'fa-circle-check';
      return `
        <tr>
          <td><strong>${p.name}</strong></td>
          <td>${p.current_stock} ${p.unit}</td>
          <td style="color:${color};font-weight:700;">
            ...
            ${p.days_until_empty} jours
          </td>
          <td>${p.avg_daily_consumption.toFixed(2)} ${p.unit}/jour</td>
        </tr>`;
    }).join('');
    ...
    const heatmapCells = heatmap.heatmap.map(d => {
      const intensity = d.total_value / maxVal;
      const bg = intensity > 0.7 ? '#173C11' : intensity > 0.4 ? '#39812F' : intensity > 0.1 ? '#a8d5a0' : '#f4f5f7';
      const textColor = intensity > 0.4 ? '#fff' : '#1a1d23';
      return `
        <div style="background:${bg};color:${textColor};border-radius:8px;padding:12px 8px;text-align:center;">
          <div style="font-weight:700;font-size:13px;">${d.day}</div>
          <div style="font-size:11px;margin-top:4px;opacity:0.85;">${d.total_value > 0 ? fmt(d.avg_daily_value)+'/j' : '—'}</div>
        </div>`;
    }).join('');
    ...
    setContent(`
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
        <div>
          <h1 style="font-size:20px;font-weight:700;">Intelligence IA</h1>
          <p style="font-size:13px;color:var(--color-text-muted);margin-top:2px;">Analyse automatique de votre business</p>
        </div>
        <div style="display:flex;gap:8px;">
          <button class="btn btn-outline" onclick="navigateTo('monthly')">
            <i class="fa-solid fa-calendar-days"></i> Comparer les mois
          </button>
          <button class="btn btn-primary" onclick="openAIAdvisor()">
            <i class="fa-solid fa-brain"></i> Conseil IA
          </button>
        </div>
      </div>
      ...
      <div class="card" style="border-left:4px solid var(--color-danger);">
        <h2 style="display:flex;align-items:center;gap:10px;">
          <i class="fa-solid fa-shield-halved" style="color:var(--color-danger);"></i>
          Détection d'anomalies
          ${anomalies.length > 0 ? `<span class="badge badge-out">${anomalies.length} suspecte${anomalies.length>1?'s':''}</span>` : '<span class="badge badge-ok">Aucune</span>'}
        </h2>
        <p style="font-size:13px;color:var(--color-text-muted);margin:6px 0 16px;">Sorties de stock anormalement élevées par rapport à la moyenne historique — indicateur de vol potentiel.</p>
        <div class="table-wrap">
          <table class="data-table">
            <thead><tr><th>Date</th><th>Produit</th><th>Quantité sortie</th><th>Référence</th><th>Anomalie</th><th>Responsable</th></tr></thead>
      ...
      <!-- PROFITABILITY -->
      <div class="card">
        <h2><i class="fa-solid fa-ranking-star" style="color:#d97706;margin-right:8px;"></i>Score de rentabilité par produit</h2>
        <p style="font-size:13px;color:var(--color-text-muted);margin:6px 0 16px;">Classement A→D basé sur le revenu généré et la marge. Concentrez vos achats sur les produits A et B.</p>
        <div class="table-wrap">
          <table class="data-table">
            <thead><tr><th>Produit</th><th>Catégorie</th><th>Revenu</th><th>Marge</th><th>Score</th><th>Pertes</th></tr></thead>
            <tbody>${profitRows}</tbody>
          </table>
        </div>
      </div>

      <!-- PREDICTIONS + HEATMAP side by side -->
      <div class="dashboard-grid">
        <div class="card">
          <h2><i class="fa-solid fa-clock" style="color:var(--color-info);margin-right:8px;"></i>Prédictions de rupture</h2>
          <p style="font-size:13px;color:var(--color-text-muted);margin:6px 0 16px;">Basé sur la consommation des 14 derniers jours.</p>
          <table class="data-table">
            <thead><tr><th>Produit</th><th>Stock</th><th>Jours restants</th><th>Consommation</th></tr></thead>
            <tbody>
              ${predRows || `<tr><td colspan="4" style="text-align:center;color:var(--color-text-muted);padding:20px;">Pas encore assez de données.</td></tr>`}
            </tbody>
          </table>
        </div>

        <div class="card">
          <h2><i class="fa-solid fa-fire" style="color:var(--color-warning);margin-right:8px;"></i>Heatmap de consommation</h2>
          <p style="font-size:13px;color:var(--color-text-muted);margin:6px 0 16px;">
            Meilleur jour: <strong>${heatmap.best_day}</strong> · Pire jour: <strong>${heatmap.worst_day}</strong>
          </p>
        ...
        <div style="background:#fff;border-radius:16px;padding:28px;width:100%;max-width:620px;max-height:85vh;overflow-y:auto;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
            <h3 style="font-size:18px;font-weight:700;display:flex;align-items:center;gap:10px;">
              <i class="fa-solid fa-brain" style="color:var(--color-accent);"></i> Agent Conseiller IA
            </h3>
            <button onclick="document.getElementById('aiModal').style.display='none'" style="background:none;border:none;font-size:20px;cursor:pointer;color:var(--color-text-muted);">✕</button>
          </div>
          <div class="form-group">
            <label>Question ou contexte supplémentaire (optionnel)</label>
            <input type="text" class="form-control" id="aiContext" placeholder="ex: On a eu un grand événement ce week-end, les chiffres sont inhabituels...">
          </div>
          <button class="btn btn-primary" style="width:100%;" id="btnGetAdvice" onclick="getAIAdvice()">
            <i class="fa-solid fa-wand-magic-sparkles"></i> Analyser et donner des conseils
          </button>
          <div id="aiAdviceResult" style="margin-top:20px;display:none;">
            <div style="background:#f8faf8;border:1px solid var(--color-border);border-radius:10px;padding:20px;font-size:14px;line-height:1.8;white-space:pre-wrap;" id="aiAdviceText"></div>
          </div>
        </div>
      </div>
    `);
...
function openAIAdvisor() {
  ...
  document.getElementById('aiAdviceResult').style.display = 'none';
}
...
async function getAIAdvice() {
  ...
  try {
    ...
    btn.disabled = false;
    btn.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> Analyser et donner des conseils';
  }
}
...
// ============================================================
//  PDF EXPORT
//  Generates a monthly report as a printable page
// ============================================================
async function exportPDF() {
  showToast('Génération du rapport PDF...');
  try {
    const [stats, analytics, profitability] = await Promise.all([
      api.get('/reports/dashboard'),
    ...
    printWindow.document.write(`
      ...
      <meta charset="UTF-8">
      <title>Rapport La Palmeraie — ${now}</title>
      <style>
        body { font-family: 'Segoe UI', sans-serif; padding: 40px; color: #1a1d23; max-width: 800px; margin: 0 auto; }
        h1 { color: #173C11; font-size: 24px; }
        h2 { color: #39812F; font-size: 16px; margin-top: 28px; border-bottom: 2px solid #e2e4e9; padding-bottom: 6px; }
        .kpi-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin: 16px 0; }
        .kpi { background: #f4f5f7; border-radius: 8px; padding: 14px; text-align: center; }
        .kpi .value { font-size: 18px; font-weight: 700; color: #173C11; }
        .kpi .label { font-size: 12px; color: #6b7280; margin-top: 2px; }
        table { width: 100%; border-collapse: collapse; font-size: 13px; margin-top: 12px; }
        th { text-align: left; padding: 8px 10px; background: #f4f5f7; font-weight: 600; }
        td { padding: 8px 10px; border-bottom: 1px solid #f3f4f6; }
        .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 28px; }
        .badge { display:inline-block;padding:2px 8px;border-radius:99px;font-size:11px;font-weight:600; }
        .badge-a { background:#e8f5e4;color:#39812F; }
        .badge-b { background:#eff6ff;color:#3b82f6; }
      ...
      <div class="header">
        <div>
          <h1>🌴 La Palmeraie</h1>
          <p style="color:#6b7280;margin-top:4px;">Rapport de gestion — ${now}</p>
        </div>
        <div style="text-align:right;font-size:13px;color:#6b7280;">
          Généré automatiquement<br>par La Palmeraie Stock Manager
        </div>
      </div>

      <h2>📊 Résumé financier</h2>
      <div class="kpi-grid">
        <div class="kpi"><div class="value">${fmt(analytics.summary.total_purchase_cost)}</div><div class="label">Coût d'achat total</div></div>
        <div class="kpi"><div class="value">${fmt(analytics.summary.total_theoretical_gain)}</div><div class="label">Gain théorique</div></div>
        <div class="kpi"><div class="value">${fmt(analytics.summary.total_real_gain)}</div><div class="label">Gain réel</div></div>
        <div class="kpi"><div class="value">${fmt(analytics.summary.total_loss_value)}</div><div class="label">Valeur des pertes</div></div>
        <div class="kpi"><div class="value" style="color:${analytics.summary.total_margin>=0?'#39812F':'#ef4444'}">${fmt(analytics.summary.total_margin)}</div><div class="label">Marge nette</div></div>
        <div class="kpi"><div class="value">${stats.total_products}</div><div class="label">Produits en stock</div></div>
      </div>

      <h2>⚠️ Alertes stock (${stats.alerts.length})</h2>
      ${stats.alerts.length === 0 ? '<p>Aucune alerte — tous les stocks sont OK.</p>' : `
      <table>
        ...
        <tbody>
          ${stats.alerts.map(a => `
            <tr>
              ...
              <td>${a.alert_threshold} ${a.unit}</td>
              <td>${a.status === 'out' ? '🔴 Rupture' : '🟡 Stock faible'}</td>
            </tr>`).join('')}
        </tbody>
      </table>`}

      <h2>🏆 Classement de rentabilité</h2>
      <table>
        <thead><tr><th>Grade</th><th>Produit</th><th>Revenu</th><th>Marge</th><th>Score</th></tr></thead>
        <tbody>
          ${profitability.slice(0,10).map(p => `
            <tr>
              <td><span class="badge badge-${p.grade.toLowerCase()}">${p.grade}</span></td>
              <td>${p.name}</td>
              <td>${fmt(p.revenue)}</td>
              <td style="color:${p.margin_pct>=0?'#39812F':'#ef4444'}">${p.margin_pct}%</td>
              <td>${p.score}/100</td>
            </tr>`).join('')}
        </tbody>
      </table>
      ...
      <p style="margin-top:40px;font-size:12px;color:#9ca3af;text-align:center;">
        La Palmeraie Stock Manager · Rapport confidentiel · ${now}
      </p>
...
async function deleteUser(userId, userName) {
  ...
  try {
    await api.delete(`/reports/users/${userId}`);
...
//  Accessible depuis Analytics — bouton "Comparer les mois"
// ============================================================
async function renderMonthlyComparison() {
  ...
  try {
    ...
    const changeChip = (pct) => {
      if (pct === null) return '<span style="color:var(--color-text-muted);font-size:12px;">—</span>';
      const up = pct >= 0;
      return `<span style="color:${up?'var(--color-success)':'var(--color-danger)'};font-weight:700;font-size:13px;">
        <i class="fa-solid fa-arrow-${up?'up':'down'}"></i> ${Math.abs(pct)}%
      </span>`;
    };

    const comparisonHtml = comparison ? `
      <div class="card" style="border-left:4px solid var(--color-accent);">
        <h2 style="margin-bottom:16px;">📊 ${comparison.current_month} vs ${comparison.previous_month}</h2>
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;">
          <div style="text-align:center;padding:12px;background:#f8faf8;border-radius:10px;">
            <div style="font-size:12px;color:var(--color-text-muted);margin-bottom:6px;">Gain réel</div>
            ${changeChip(comparison.revenue_change)}
          </div>
          <div style="text-align:center;padding:12px;background:#f8faf8;border-radius:10px;">
            <div style="font-size:12px;color:var(--color-text-muted);margin-bottom:6px;">Coût d'achat</div>
            ${changeChip(comparison.cost_change)}
          </div>
          <div style="text-align:center;padding:12px;background:#f8faf8;border-radius:10px;">
            <div style="font-size:12px;color:var(--color-text-muted);margin-bottom:6px;">Marge</div>
            ${changeChip(comparison.margin_change)}
          </div>
          <div style="text-align:center;padding:12px;background:#f8faf8;border-radius:10px;">
            <div style="font-size:12px;color:var(--color-text-muted);margin-bottom:6px;">Pertes</div>
            ${changeChip(comparison.loss_change)}
          </div>
        </div>
      </div>` : '';
    ...
    const tableRows = months.map(m => `
      <tr>
        ...
        <td style="color:var(--color-danger);">${fmt(m.purchase_cost)}</td>
        <td style="color:var(--color-info);">${fmt(m.theoretical_gain)}</td>
        <td style="color:var(--color-warning);">${fmt(m.loss_value)}</td>
        <td style="color:var(--color-accent);font-weight:600;">${fmt(m.real_gain)}</td>
        <td style="color:${m.margin>=0?'var(--color-success)':'var(--color-danger)'};font-weight:700;">${fmt(m.margin)}</td>
      </tr>`).join('');

    setContent(`
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
        <div>
          <h1 style="font-size:20px;font-weight:700;">Comparaison mensuelle</h1>
          <p style="font-size:13px;color:var(--color-text-muted);margin-top:2px;">6 derniers mois</p>
        </div>
        <button class="btn btn-outline" onclick="navigateTo('analytics')">
          <i class="fa-solid fa-arrow-left"></i> Retour Analytics
        </button>
      </div>
      ...
      <div class="card">
        <h2><i class="fa-solid fa-chart-bar" style="color:var(--color-accent);margin-right:8px;"></i>Évolution sur 6 mois</h2>
        <br>
        <canvas id="monthlyChart" height="90"></canvas>
      </div>

      <div class="card">
        <h2><i class="fa-solid fa-table" style="color:var(--color-text-muted);margin-right:8px;"></i>Tableau détaillé</h2>
        <div class="table-wrap" style="margin-top:16px;">
          <table class="data-table">
            <thead>
              <tr>
                <th>Mois</th>
                <th style="color:var(--color-danger);">Coût achat</th>
                <th style="color:var(--color-info);">Gain théorique</th>
                <th style="color:var(--color-warning);">Pertes</th>
                <th style="color:var(--color-accent);">Gain réel</th>
                <th>Marge</th>
              </tr>
            </thead>
      ...
      new Chart(canvas, {
        ...
        data: {
          ...
          datasets: [
            {
              label: "Coût d'achat",
              data: months.map(m => m.purchase_cost),
              backgroundColor: 'rgba(239,68,68,0.7)',
              borderRadius: 4,
            },
            {
              label: 'Gain réel',
              data: months.map(m => m.real_gain),
              backgroundColor: 'rgba(57,129,47,0.7)',
              borderRadius: 4,
            },
            {
              label: 'Marge',
